...
class ParentGuardian(models.Model):
    ...
    def save(self, *args, **kwargs):
        ...
        if password_missing:
            ...
            # set a temporary cleartext password now; we'll hash before saving
        ...
        # Ensure password is hashed before saving. If the stored password already
        # appears to be hashed (has an identifiable hasher), leave it as-is.
        pw = getattr(self, 'password', None)
        if pw:
            try:
                # identify_hasher will raise a ValueError if 'pw' is not a valid encoded password
                identify_hasher(pw)
                is_hashed = True
            except Exception:
                is_hashed = False
